...
Beyond forgetting the past states, the ESP is also often discussed in terms of a stability property termed input-related stability where nearby inputs create nearby responses (solutions)~\cite{manjunath2020stability}. 
...
This could possibly complicate our task. %\ednote{B: Grammarly}
...
\begin{figure}[ht]
  \includegraphics[width=0.5\linewidth]{Graphs/_Nonlinearity_Fig_response.eps}
  \centering
...
% One need therefore not establish any additional results to ensure that the sequence uniformly approaches the unique solution $\Psi$. This vastly simplifies the effort necessary to set up a problem to guarantee that the underlying system will be accurately represented by the conjugate system.
...
%\begin{Theorem}
 %   $(Y_T, G_T)$ is semi-conjugate to $(\widehat{U}_T, \widehat{T})$.
%\end{Theorem}
...
% Given certain assumptions, we can guarantee that $(\widehat{U}_T, \widehat{T})$ is at least semi-conjugate to $(U,T)$.
