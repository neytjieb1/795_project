...
In this method, one considers a discrete-time dynamical system defined on a 'nice' space (a smooth manifold)\ednote{Should this be further explained?} that can be obtained as time-$K$ map of a continuous time-dynamical system, concepts which will be formalised below.
...
\begin{figure}[ht]
  ...
  \label{fig:takensmap}
\end{figure}
\ednote{M: Here, and throughout if you reproduce a figure, it must be stated where it was reproduced from. B: Is this satisfactory? Also, should $\tilde{F}$ be left out for now?}
\ednote{B: This is only a screenshot used as a temporary placeholder. If sufficient, I shall convert it to .eps. If not, may I perhaps have the original picture?}
...
% \ednote{Add Causal Embedding Theorem}
...
It may easily be verified that a DP undergoing damping (as seen in \ref{conjugacy_with_Gamma}) is not surjective and hence the required assumptions to apply our theory do not hold. \ednote{B: Change sentence.}
...
\begin{figure}[ht]
  ...
  \begin{figure}[ht]
    \centering 
  \includegraphics[width=\linewidth]{_DP_Fig-2D_5.eps}
...
\end{center}
\ednote{B: Functional Complexity will be explained before this}
