...
\begin{Definition}
  [\bf Flow of an Equation] \label{Dfn_Flow}
...
\begin{Definition}
  [\bf Invariant Set] \label{Dfn_Flow}
...
\begin{Definition} 
  [\bf Attractor] \label{Dfn_Attractor}
...
\begin{Definition}
  [\bf {Sensitive Dependence on Initial Conditions}] \label{Dfn_SDIC}
...
\begin{Definition}
  [\bf {Topologically Transitive}] \label{Dfn_TopTrans}
...
\begin{Definition} 
  [\bf {Meager Set}] \label{Dfn_Meager Set}
...
\begin{Definition}
  [\bf {Devaney's Chaos}]\label{Dfn_ChaosDec}
...
\begin{Definition}  
  [\bf {Conjugacy \& Semi-Conjugacy}]\label{Dfn_Conjugate}
...
\begin{Definition}
  [\bf Nonautonomous Dynamical System (adopted from~\cite{Manju_ESP}))] \label{Dfn_Soln}\rm
  A nonautonomous dynamical system (NDS) on a space $X$ is simply a dynamical system comprising a family of maps $\{g_n\}_{n \in \mathbb{Z}}$ where $g_n(\ldotp):=g(u_n,\ldotp):X\to{X}$ is continuous. Each $g_n$ arises as a consequence of an input $u_n$ from the input space $U$, a topological space.  \ednote{Does the reader immediately remember/know what is meant by a dynamical system?}
\end{Definition}
...
This will lead to a result established later, but which is worth taking note of now: \textit{$g$ being a topological contraction is equivalent to the existence of a unique entire solution.} To this end we define the notion of a topological contraction. \ednote{M: I do not think you need the definition of a topological contraction unless it is defined through a process. Please note that you have defined $X_n(\bar{u})$ differently than what was in the earlier works. Now it is just the collection of solution values at time $n$ with the input $\bar{u}$ and topological contraction is just paraphrasing the USP definition. B: Is it better to then just leave out the entire sentence?}
...
\end{Theorem}
\begin{proof}
  \textbf{I've muddled myself here.}
  % Consider an input $\overline{u}={\{u_n\}}_{n\in\mathbb{Z}}\subset{U}$ and driven system $g:U\times{X}\to{X}$  generating a sequence ${\{x_n\}}_{n\in\mathbb{Z}}$ in the compact space $X$ \ednote{This statement is not correct.}. 
...
\textbf{NEED TO PARAPHRASE}
\newline In our context, $g$ having the USP is equivalent to $g$ being a topological contraction \cite{manjunath2021universal}, or alternatively that $g$ exhibits the Echo State Property(ESP). 
In \cite{Manju_ESP} the echo state property is a key for the design and training of recurrent neural networks within the paradigm of reservoir computing.
Available definitions and theorems concerning the echo state property, however, are of little practical use because they do not relate the network response to temporal or statistical properties of the driving input.

A greater discussion is presented in \cite{jaeger2001echo}
a particular stability concept for which a number of equivalent definitions are available \cite{jaeger2001echo}.

In most of the reservoir computing (RC) literature, the idea of memory-loss or fading memory popularly called
the echo state property (ESP) is treated as a stability property of the system valid
for all inputs. In such a case, the ESP guarantees that the entire past history of the
input determines the state of the system precisely, i.e., there is no possibility of two
or more reachable states at any given point in time [5] if the entire past values of the
input have influenced the dynamics. The formal definition linking ESP intrinsically
to an input is available in [4].

In the context of information processing, when a driven system g has a causal em-
bedding, it helps carry over any input ū onto the solution space without distortion or
loss of information. This is a very important feature, and the current discrete-time
state space models used in reservoir computing do not exploit the causal embedding.
We intend to demonstrate the utility of these theoretical results in a future work
directed at applications.

For natural systems, say, for instance, when the human brain is viewed as a driven
system where a time-varying external stimulus (for instance, a visual stimulus) is
an input, the input can be causally embedded into the nervous system without the
need for a particular structure that is custom-made for each stimulus as long as
the system has the USP. From the perspective of perturbation of an autonomous
dynamical system, if the resulting nonautonomous system has the USP, then the
temporal structure of noise relates to the dynamics on a nonautonomous uniform
attractor through a topological semi-conjugacy or a conjugacy.

The continuity of E reflects on whether small changes in u ~ would result in a ‘propor-
tionate’ response in the solution space – a notion of input related stability as defined in \cite{Manju_ESP}

To understand
how solutions become sensitive to a perturbation of the input for systems without
USP, we define a stability notion that describes how the collection of all solutions
obtained from an input ū are mapped onto the solution space.

Sufficient to consider continuity of encoding map.

owing to the USP of the driven system $g$ one gains input-related, parameter-related stability \cite{manjunath2020stability}
...
\textbf{make special note of a} %\ednote{something you want to fill?} }
\begin{Definition}
...
\textbf{When considering the map $H$, we once again restrict restrict our focus of $H$ to the function $H_2(\overline{u}):=(h(r\overline{u}, h(\overline{u})))$. }
...
\end{equation} 
is both SI-invertible and possesses the USP. 
...
The proof of these $g$ possessing the USP is rather involved and hence the proof, instead of being replicated here, may be found in \textbf{(cite relevant article)}
...
As the Unique Attraction Property guarantees that all trajectories \ednote{(B: which ones?)} will converge to the same trajectory as time moves forward. 
...
\textbf{From the perspective of perturbation of an autonomous
dynamical system, if the resulting nonautonomous system has the USP, then the
temporal structure of noise relates to the dynamics on a nonautonomous uniform
attractor through a topological semi-conjugacy or a conjugacy.}
...
\end{Theorem}
\begin{proof}
  The proof may be found in~\cite[Th. 3c]{manjunath2021universal}.
\end{proof}
...
Thirdly, $\Gamma$ is globally dissipative whereas $G_T$ is not guaranteed to be(\emph{Cite}). \ednote{B: Add linking sentence.} Global dissipativity prevents large numerical errors due to input data.
...
In this section we turn to the implementation of the function $G_T$ by discussing a method to learn the function $\Gamma$. We then indicate where this implementation differs from previous work done by (\cite{manjunath2021universal}) and show that our results do hold water by considering the Pearson correlation coefficient. \ednote{(this sentence needs to be changed) Are you talking of MDD? Or the sparse matrix?.} 
...
The map $G_T$ is implemented by applying the functions $\Gamma$, $g$ and the projection function $\pi_2$ in the relevant order. To reach this point, one must first learn $\Gamma$.
After 1.normalising the data to ensure it lies within the interval $|x| < 1$ and has zero mean value, we 2.drive the system with external input through the function $g$.

From before (\eqref{eqn_driving}): $ g(u,x)=(1-a)x+a{\cdot}\tanh(Au+\alpha{a}Bx)$. Remember that $g$ is SI-invertible. $A$ is the so-called “input matrix” and $B$ the “reservoir matrix”. 
...
A complete discussion of the RNN is outside the scope of this thesis, so we only present the details necessary to the current discussion. For information, (\textbf{Cite, Cite, Cite}) may be consulted.

If the RNN we will be using in the next step has $N$ neurons, then $A$ and $B$ are each $N\times{N}$ matrices. Inputs $u_n$ of dimension $K < N$ are padded by zeroes to obtain an input of dimension $N$.

No need for entire left-infinite history of input: An arbitrary initialisation of the system with initial values and the driving of the system with the function $g$ means that the generated values will come to approximate the actual solution’s elements in a uniform manner thanks to the UAP.

We then 3.set up/prepare the RNN. Utilising a network $N$ neurons (the same $N$ as in the dimensions of the matrices $A,B$). Training is accomplished using the Adam Optimiser where the MSE loss function is optimised. The system is trained several times(in our case 4) with incrementally smaller learning rates on each run (0.001 and thereafter divided by 10 at each run). 
\ednote{Please present the number of layers in the network, number of neurons in each layer and the length of the training validation and testing for learning Gamma.}
...
The two model equations for learning from data (\eqref{eqns_from_data}) also allows us to determine what the state $x_{n+1}$ at time $n+1$ will be. 
...
\begin{itemize}
  \item What it is. 
  \item Why it's chaotic
  \item How data was generated for this. (noiseless)
...
An ideal system is assumed where the rods are massless and no friction in the pivots are considered.
One is easily reminded of the Lorenz system, another three-equation deterministic model which exhibits chaotic behaviour.
The scalar equations of motion are presented below and were derived with the use of Newtonian physics and can be found in \textbf{(Cite.)}
...
This second-order system was then converted into a first-order system by defining the first derivatives as separate values.`'
...
\item Clifford:
\begin{equation}\label{eqn_clifford}
  \frac{\sin(ay) + c\cos(ax)}{\sin(bx)+d\cos(by)}
\end{equation}
\end{itemize}


\section{Generalized Pearson correlation coefficient for the DP and other attractors}
\textbf{Insert table: D.Pendulum, Lorenz, Others (Clifford \& Thomas)}
