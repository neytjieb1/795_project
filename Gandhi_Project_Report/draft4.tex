...
\begin{abstract}
  Collecting measurements from underlying artificial, natural, and physical systems through observables to realise a dynamical model with greater descriptive power is a well-established practice in present-day science. However, we often do not fully understand the many facets of the more complicated systems we interact with on a daily basis.

  In this project we formulate the concept of discrete-time dynamical systems, particularly those which are chaotic. The learning problem is formulated and a well-known approach (first proposed by Takens) to solving the problem alongside its limitations is considered.
  We then introduce the notion of driven dynamical systems and establish the theoretical existence of dynamical systems possessing certain properties that guarantee them to be topologically conjugate (or dynamically equivalent) to the dynamics of the underlying system from which measurements were taken. The methodology is practically implemented by making use of a recurrent neural network, a form of deep learning within the reservoir computing framework and we show that we may obtain long-term consistent predictions for simple physical systems such as the double pendulum as well as attractors exhibiting chaotic behaviour.
...
\end{proof}
\ednote{M: this sentence is freely hanging} To establish whether $\widehat{T}$ and $G_T$ are related, we consider the the function $H_2{\overline{}} := (h(r\overline{u}, h(\overline{u})))$ where $H_2$ is a mapping from the entirety of a left-infinite sequence to some element of $X\times{X}$.
...
\begin{figure}[ht]
  ...
  \centering
\captionof{figure}{Illustration of trajectories that all approach the true solution if $g$ has the UAP. Reproduced from \cite{manunath2021universal}.}
...
\begin{figure}[ht]
  \includegraphics[scale=0.3]{_autovsdriven.eps}
  \centering
  \label{fig:learningFailure}
  \captionof{figure}{Illustrating empirically the importance of global dissipativity. Plotted below the three principal components of randomly initiated points(blue) of a learnt version of $G_T$ versus the trajectory $(x_{n-1}, x_n)$(red) of driven states obtained from the Fractal Dream Attractor's data~\ref{Thomas_Attractor}). Figure generated by altering the code used to produce Figure 7 in \cite{Supp}. }
\end{figure}
